...
export default function WhatsAppButton({ cake, orderData, isCustom }: Props) {
  const generateMessage = () => {
    const businessName = process.env.NEXT_PUBLIC_BUSINESS_NAME;
    
    if (isCustom) {
      return `Hi ${businessName}, I&apos;m interested in a custom cake. Request ID: ${orderData.orderId}`;
    }

    if (cake) {
      return `Hi ${businessName}, I want to order:
Cake: ${cake.name}
Price: ₹${cake.basePrice}
Please confirm availability and total amount.`;
    }

    return `Hi ${businessName}, I want to place an order.`;
  };
